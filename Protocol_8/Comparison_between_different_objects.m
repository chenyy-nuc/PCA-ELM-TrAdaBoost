...
for m2 = 20:5:60
    ...
    T_train_target = propvals.data(k(1:m2),4);  % protein component
    m2 = size(T_train_target,1);
    % 目标领域测试样本
    P_test_target = mp5spec.data(k(m2+1:m2+m3),1:2:end);
    T_test_target = propvals.data(k(m2+1:m2+m3),4);
    m3 = size(T_test_target,1);
    % 源领域训练样本（辅助训练样本）
%     P_train_source = mp5spec.data(k(m2+m3+1:m2+m3+m1),1:2:end);
%     T_train_source = propvals.data(k(m2+m3+1:m2+m3+m1),4);
%     T_train_source = mapminmax(T_train_source',7,10);
%     T_train_source = T_train_source';
    
    load spectra.mat
    P_train_source = NIR(:,52:end);
    T_train_source = octane;
    T_train_source = mapminmax(T_train_source',62,67);
    T_train_source = T_train_source';
